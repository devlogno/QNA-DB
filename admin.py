...
from models import db, Question, ReportedQuestion, User, Notification, NewsArticle
from models import Level, Stream, Board, Subject, Chapter, Topic # Import new models
...
def manage_categories():
    """Renders the main page for managing the category hierarchy."""
...
def add_question():
    """
    Handles adding a new MCQ or CQ question with full hierarchical categorization.
    """
    if request.method == 'POST':
        try:
            ...
            # --- Process MCQ/CQ Specific Fields ---
            ...
            print(f"Error adding question: {e}") # For debugging
...
def add_news():
    if request.method == 'POST':
        ...
        new_article = NewsArticle(title=title, content=content, for_ssc=for_ssc, for_hsc=for_hsc)
...
def get_children(model, parent_id_field, parent_id):
    """
    Fetches child items for a given model and parent.
    """
    query = model.query
    # If a parent is specified, apply the filter.
    if parent_id and parent_id_field:
        query = query.filter(getattr(model, parent_id_field) == parent_id)
    
    # Execute the query to get the list of items.
    items = query.all()
...
def delete_item(model, item_id):
    item = model.query.get_or_404(item_id)
    child_relation_name = model.__tablename__ + 's' 
    if hasattr(item, child_relation_name) and getattr(item, child_relation_name).first():
        return jsonify({'success': False, 'message': f'Cannot delete. This {model.__name__} has child items.'}), 400
    try:
...
def handle_categories(category_name):
    ...
    if request.method == 'GET':
        ...
        return get_children(model, parent_id_field, parent_id)
...
def handle_category_item(category_name, item_id):
    ...
    if request.method == 'PUT':
        return update_item(model, item_id)
...
# NEW ROUTE: Re-implementation of edit_question
