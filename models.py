...
# --- Association Table for User <-> Level Many-to-Many relationship ---
user_levels = db.Table('user_levels',
...
class Stream(db.Model): # e.g., Science, Arts, Commerce
    id = db.Column(db.Integer, primary_key=True)
...
class Board(db.Model): # e.g., Dhaka Board, BUET
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    tag = db.Column(db.String(20), nullable=True) # ADDED: New editable tag field
...
class User(UserMixin, db.Model):
    ...
    levels = db.relationship('Level', secondary=user_levels, lazy='subquery',
...
class Question(db.Model):
    ...
    question_type = db.Column(db.String(10), nullable=False) # MCQ or CQ
    year = db.Column(db.Integer, nullable=False)
    complexity = db.Column(db.Integer, nullable=False, default=5) # Complexity from 1 to 10

    # Foreign Keys for the new hierarchy
    level_id = db.Column(db.Integer, db.ForeignKey('level.id'), nullable=False)
...
# --- Other Models (largely unchanged) ---
